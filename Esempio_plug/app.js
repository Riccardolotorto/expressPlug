...
app.get('/profile', (req, res) => {
    const person = people.profiles.find((p) => p.id === req.query.id);  //Viene individuata la persona con l’id specificato nell’url. Con req.query otterremo {name:"tom", age: "55"} 
    ...
    res.render('profile', {
        title: `About ${person.firstname} ${person.lastname}`, //A questo punto renderizzeremo la pagina pug profile passando i dati che ci interessano (in questo caso il titolo e i dati sulla persona)
        person,
    });
});
